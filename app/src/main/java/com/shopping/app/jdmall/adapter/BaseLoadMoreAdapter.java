...
import com.shopping.app.jdmall.widget.LoadMorePress;
...
public abstract class BaseLoadMoreAdapter<T> extends BaseListMoreAdapter<T> {

    private static final int ITEM_TYPE_NORMAL = 0;
    private static final int ITEM_TYPE_LOAD_MORE = 1;

    public BaseLoadMoreAdapter(Context context, List list) {
        super(context, list);
    }
    ...
    protected ViewHolder onCreateViewHolder(int position) {
        if(getItemViewType(position) == ITEM_TYPE_NORMAL) {
            ...
            return new ViewHolder(new LoadMorePress(getContext()));
        }
    }
